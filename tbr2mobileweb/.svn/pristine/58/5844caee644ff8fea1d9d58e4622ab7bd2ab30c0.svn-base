...
import org.irdresearch.tbreach2.shared.model.Surveys;
...
/*	try
		...
		else if (reqType.equals (RequestType.PATIENT_REGISTRATION))
		{
			return doPatientRegistration ();
		}
		
		else if (reqType.equals (RequestType.BASELINE_FORM))
		{
			return doBaseline ();
		}
		
		else if (reqType.equals (RequestType.MONITORING_FORM))
		{
			return doMonitoringFormInfo ();
		}
		
		else if (reqType.equals(RequestType.SURVEY_FORM))
		{
			return doSurveyForm ();
		}
...
/*			if (!StringUtils.isEmptyOrWhitespaceOnly (labId))
	...
	private String doSurveyForm (){
		
		String patientID=request.getParameter("parId");
		String startDate =request.getParameter("startdate");
		String startTime = request.getParameter ("stime");
		String endTime = request.getParameter ("etime");
		String healthWorker = request.getParameter ("healthWorker");
		
		String satisfiedWithSystem = request.getParameter ("satisfiedwithsystem");
		String unsatisfactoryAspectSystem = request.getParameter ("unsatisfactoryaspectsystem");
		String systemImproved = request.getParameter ("systemimproved");
		String satisfiedReminderSystem = request.getParameter ("satisfiedremindersystem");
		String unsatisfactoryAspectreminderSystem = request.getParameter ("unsatisfactoryaspectremindersystem");
		String smsSystemEffort = request.getParameter ("smssystemeffort");
		String smsSystemCompliance = request.getParameter ("smssystemcompliance");
		String serviceHealthFacility = request.getParameter ("servicehealthfacility");
		String careTbSpecialist = request.getParameter ("caretbspecialist");
		String tbTreatmentSystem = request.getParameter ("tbtreatmentsystem");
		...
		try
		{
			Users users = ssl.findUser (healthWorker);
			pid = users.getPid ();		
		}
		catch (Exception e)
		{
		...
		if (lab != null)
		{
			locationId = lab.getId ().getLocationId ();
		}
		...
		try
		{
			screening = ssl.findScreeningByPatientID(patientID);
		}
		catch (Exception e)
		{
			e.printStackTrace ();
			return XmlUtil.createErrorXml ("SCREENING_FORM_NOT_FOUND"+"/"+patientID);
		}
		...
		try
		{
			patientDetails = ssl.findPatientByPatientID(patientID);
		}
		catch (Exception e) {
			
			XmlUtil.createErrorXml("PATIENT_DETAILS_NOT_FOUND"+"/"+patientID);
		}
		
		if(patientDetails == null)
		{
			return XmlUtil.createErrorXml ("PATIENT_DETAILS_NOT_FOUND"+"/"+patientID);
		}

		Surveys surForm = new Surveys();
		surForm.setPid(patientID);
		surForm.setHealthWorkerID(healthWorker);
		
		try {
			surForm.setStartTimeForm(DateTimeUtil.getDateFromString (startDate+" "+startTime, DateTimeUtil.FE_FORMAT));
			surForm.setEndTimeForm(DateTimeUtil.getDateFromString (startDate+" "+endTime, DateTimeUtil.FE_FORMAT));
		} catch (ParseException e) {
			e.printStackTrace();
			return XmlUtil.tajikErrorLoggingXml ("Date parse error");
		}
		
		
		surForm.setSatisfiedWithSystem(translateSurveySelectedOption(satisfiedWithSystem));
		surForm.setUnsatisfactoryAspectSystem(unsatisfactoryAspectSystem);
		surForm.setSystemImprovement(systemImproved);
		surForm.setSatisfiedWithSmsReminder(translateSurveySelectedOption(satisfiedReminderSystem));
		surForm.setUnsatisfactoryAspectSmsReminder(unsatisfactoryAspectreminderSystem);
		surForm.setReminderAdditionalEffort(translateSurveySelectedOption(smsSystemEffort));
		surForm.setReminderTreatmentCompliance(translateSurveySelectedOption(smsSystemCompliance));
		surForm.setSatisfiedServiceHealthFacility(translateSurveySelectedOption(serviceHealthFacility));
		surForm.setSatisfiedCareTbSpeaclist(translateSurveySelectedOption(careTbSpecialist));
		surForm.setTbTreatmentSystem(translateSurveySelectedOption(tbTreatmentSystem));
		...
		ssl.save(surForm);
			...
			e.printStackTrace();
			return XmlUtil.createErrorXml ("ERROR_IN_SAVING_FORM");
		}
		
		Date encounterdateentered = null;
		Date encounterStartDate = null;
		Date encounterEndDate = null;
		try
		{
			encounterdateentered = DateTimeUtil.getDateFromString (startDate+" "+startTime, DateTimeUtil.FE_FORMAT);
			encounterStartDate   = DateTimeUtil.getDateFromString (startDate+" "+startTime, DateTimeUtil.FE_FORMAT);
			encounterEndDate   	 = DateTimeUtil.getDateFromString (startDate+" "+endTime, DateTimeUtil.FE_FORMAT);
		}
		catch (Exception e1)
		{
			e1.printStackTrace ();
			return XmlUtil.createErrorXml ("DATE_ERROR_ENCOUNTER");
		}
		
		EncounterId encId = new EncounterId (0, patientID.toUpperCase(), healthWorker.toUpperCase());
		

		Encounter e = new Encounter (encId, EncounterType.SURVEY_FORM, locationId, encounterdateentered, encounterStartDate, encounterEndDate, "");
		...
		{
			e1.printStackTrace ();
			XmlUtil.createErrorXml ("SAVE_ENCOUNTER_ERROR");
		}
		
		ArrayList<EncounterResults> encounters = new ArrayList<EncounterResults> ();

		EncounterResults systemSatisfictaion = ModelUtil.createEncounterResult (e, "satisfication_system".toUpperCase (), surForm.getSatisfiedWithSystem());
		encounters.add (systemSatisfictaion);
		
		EncounterResults unsatisfactoryApectSystem = ModelUtil.createEncounterResult (e, "unsatisfactory_aspect_system".toUpperCase (), surForm.getUnsatisfactoryAspectSystem());
		encounters.add (unsatisfactoryApectSystem);
		
		EncounterResults systemImprovment = ModelUtil.createEncounterResult (e, "system_improvement".toUpperCase (), surForm.getSystemImprovement ());
		encounters.add (systemImprovment);
		
		EncounterResults reminderSystemSatisfication = ModelUtil.createEncounterResult (e, "satisfication_reminder_system".toUpperCase (), surForm.getSatisfiedWithSmsReminder());
		encounters.add (reminderSystemSatisfication);

		EncounterResults additionalEffort = ModelUtil.createEncounterResult (e, "additional_effort".toUpperCase (), surForm.getReminderAdditionalEffort ());
		encounters.add (additionalEffort);
		
		EncounterResults treatmentCompliance = ModelUtil.createEncounterResult (e, "treatment_compliance".toUpperCase (), surForm.getReminderTreatmentCompliance());
		encounters.add (treatmentCompliance);
		
		EncounterResults healthFacilityService = ModelUtil.createEncounterResult (e, "health_facility_service".toUpperCase (), surForm.getSatisfiedServiceHealthFacility ());
		encounters.add (healthFacilityService);
		
		EncounterResults tbSpecalistCare = ModelUtil.createEncounterResult (e, "tb_speaclist_care".toUpperCase (), surForm.getSatisfiedCareTbSpeaclist ());
		encounters.add (tbSpecalistCare);
		
		EncounterResults tbTreatment = ModelUtil.createEncounterResult (e, "tb_treatment_system".toUpperCase (), surForm.getTbTreatmentSystem ());
		encounters.add (tbTreatment);
		...
		for (int i = 0; i < encounters.size (); i++)
		{
			...
			if (!resultSave)
			{
				return XmlUtil.createErrorXml ("SAVE_ENCOUNTER_RESULT_ERROR");
			}
		}
	...
	private String doMonitoringFormInfo ()
	{

			String patientID=request.getParameter("parId");
			String startDate =request.getParameter("startdate");
			String startDateTreatment = request.getParameter ("startTreatment");
			String startTime = request.getParameter ("stime");
			String endTime = request.getParameter ("etime");
	/*		String monthSmear2 = request.getParameter ("monthSmear2");
			String monthSmear3 = request.getParameter ("monthSmear3");
			String monthSmear4 = request.getParameter ("monthSmear4");
			String monthSmear5 = request.getParameter ("monthSmear5");
			String monthSmear6 = request.getParameter ("monthSmear6");
			String monthSmear7 = request.getParameter ("monthSmear7");
			String monthSmear6Cat2 = request.getParameter ("monthSmear6Cat2");
	*/		String smearResult = request.getParameter ("smearResult");
			String monthTreatment = request.getParameter ("monthTreatment");
			String healthWorker = request.getParameter ("healthWorkerID");
			String baselineSmearResult = request.getParameter ("baselineSmearResult");
			
			
			String patientFeel = request.getParameter ("patientFeel");
			String experiencedSideEffect = request.getParameter ("experiencedSideEffect");
			String patientSideEffects = request.getParameter ("patientSideEffects");
			String otherSideEffects = request.getParameter ("otherSideEffects");
			String patientConsult = request.getParameter ("patientConsult");
			String healthFacility = request.getParameter ("healthFacility");
			String missMedication = request.getParameter ("missMedication");
			String durationMissMedication = request.getParameter ("durationMissMedication");
			String medicineWrappers = request.getParameter ("medicineWrappers");
			...
			try
			{
				long moncount = ssl.count ("monitoringresults", "where PatientID='" + patientID + "' AND CurrentMonth='"+monthTreatment+"'");
				
				//MonitoringResults mresults = ssl.findMonitoringResultsByPatientIDAndMonth(patientID,monthTreatment);
				if (moncount != 0)
					return XmlUtil.createErrorXml ("TREATMENT_MONTH_ALREADY_FOUND"+"/"+monthTreatment);
			}
			catch (Exception e)
			{
				e.printStackTrace ();
			...
			MonitoringResults mr = new MonitoringResults();
			mr.setPatientId(patientID);
			
			if(areRussianStringsEqual(baselineSmearResult,"Отр"))
			...
			if(areRussianStringsEqual(smearResult,"Отр"))
		...
		/*	if(areRussianStringsEqual(monthSmear2,"Манфи"))
				mr.setSmearResultMonth2("NEGATIVE");
			else if (areRussianStringsEqual(monthSmear2,"1-9 КУБ"))
				mr.setSmearResultMonth2("1-9 AFB");
			else
				mr.setSmearResultMonth2(monthSmear2);
			
			if(areRussianStringsEqual(monthSmear3,"Манфи"))
				mr.setSmearResultMonth3("NEGATIVE");
			else if (areRussianStringsEqual(monthSmear3,"1-9 КУБ"))
				mr.setSmearResultMonth3("1-9 AFB");
			else
				mr.setSmearResultMonth3(monthSmear3);
			
			if(areRussianStringsEqual(monthSmear4,"Манфи"))
				mr.setSmearResultMonth4("NEGATIVE");
			else if (areRussianStringsEqual(monthSmear4,"1-9 КУБ"))
				mr.setSmearResultMonth4("1-9 AFB");
			else
				mr.setSmearResultMonth4(monthSmear4);
			
			if(areRussianStringsEqual(monthSmear5,"Манфи"))
				mr.setSmearResultMonth5("NEGATIVE");
			else if (areRussianStringsEqual(monthSmear5,"1-9 КУБ"))
				mr.setSmearResultMonth5("1-9 AFB");
			else
				mr.setSmearResultMonth5(monthSmear5);
			
			if(areRussianStringsEqual(monthSmear6,"Манфи"))
				mr.setSmearResultMonth6("NEGATIVE");
			else if (areRussianStringsEqual(monthSmear6,"1-9 КУБ"))
				mr.setSmearResultMonth6("1-9 AFB");
			else
				mr.setSmearResultMonth6(monthSmear6);
			
			mr.setBaselineSmear(baselineSmearResult);
			mr.setCurrentMonth(Integer.parseInt(monthTreatment));
			
			if(areRussianStringsEqual(monthSmear6Cat2,"Манфи"))
				mr.setSmearResultMonth6Cat2("NEGATIVE");
			else if (areRussianStringsEqual(monthSmear6Cat2,"1-9 КУБ"))
				mr.setSmearResultMonth6Cat2("1-9 AFB");
			else
				mr.setSmearResultMonth6Cat2(monthSmear6Cat2);
			
			if(areRussianStringsEqual(monthSmear7,"Манфи"))
				mr.setSmearResultMonth7("NEGATIVE");
			else if (areRussianStringsEqual(monthSmear7,"1-9 КУБ"))
				mr.setSmearResultMonth7("1-9 AFB");
			else
				mr.setSmearResultMonth7(monthSmear7);
			
			if(areRussianStringsEqual(monthSmear9,"Манфи"))
				mr.setSmearResultMonth9("NEGATIVE");
			else if (areRussianStringsEqual(monthSmear9,"1-9 КУБ"))
				mr.setSmearResultMonth9("1-9 AFB");
			else
				mr.setSmearResultMonth9(monthSmear9);
			*/
			mr.setHealthWorkerId(healthWorker);
			
			if(areRussianStringsEqual(patientFeel,"Улучшение"))
				mr.setPatientFeel("BETTER THAN BEFORE");
			else if (areRussianStringsEqual(patientFeel,"Ухудшение"))
				mr.setPatientFeel("WORSE THAN BEFORE");
			else if (areRussianStringsEqual(patientFeel,"Без изменений"))
				mr.setPatientFeel("SAME AS BEFORE");
			else
				mr.setPatientFeel("NOT SURE");
			
			
			if(areRussianStringsEqual (experiencedSideEffect, "Да"))
				mr.setExperienceSideEffects("YES");
			else 
				mr.setExperienceSideEffects("NO");
			
			if(mr.getExperienceSideEffects ().equals ("YES")){
				...
				mr.setPatientSideEffects (patientSideEffectFull.substring (0, patientSideEffectFull.length ()-1));
			}
			
			if(areRussianStringsEqual (patientConsult, "Да"))
				mr.setPatientConsult("YES");
			else 
				mr.setPatientConsult("NO");
			
			if(mr.getPatientConsult ().equals ("YES")){
				...
				mr.setHealthFacility (healthFacilityFull.substring (0, healthFacilityFull.length ()-1));
			}
			
			if(areRussianStringsEqual (missMedication, "Да"))
				mr.setMissMedication("YES");
			else 
				mr.setMissMedication("NO");
			
			if(mr.getMissMedication ().equals ("YES")){
				if(areRussianStringsEqual(durationMissMedication,"один раз"))
					mr.setDurationMissMedication("ONCE");
				else if (areRussianStringsEqual(durationMissMedication,"два раза"))
					mr.setDurationMissMedication("TWICE");
				else if (areRussianStringsEqual(durationMissMedication,"три раза"))
					mr.setDurationMissMedication("THRICE");
				else if (areRussianStringsEqual(durationMissMedication,"неделю"))
					mr.setDurationMissMedication("FOR A WEEK");
				else if (areRussianStringsEqual(durationMissMedication,"более недели"))
					mr.setDurationMissMedication("MORE THAN A WEEK");
				else
					mr.setDurationMissMedication("NOT SURE");
			}
			
			if(areRussianStringsEqual (medicineWrappers, "Да"))
				mr.setMedicineWrappers("YES");
			else 
				mr.setMedicineWrappers("NO");
		
			if(mr.getMedicineWrappers ().equals ("YES"))
				mr.setDoseConsumption(doseConsumption);
			
			mr.setCurrentMonth (Integer.parseInt (monthTreatment));
			
			try {
				mr.setDateofTreatment(DateTimeUtil.getDateFromString (startDateTreatment, DateTimeUtil.FE_FORMAT_TAJIKISTAN));
				mr.setStartTimeForm(DateTimeUtil.getDateFromString (startDate+" "+startTime, DateTimeUtil.FE_FORMAT));
				mr.setEndTimeForm(DateTimeUtil.getDateFromString (startDate+" "+endTime, DateTimeUtil.FE_FORMAT));
			} catch (ParseException e) {
				e.printStackTrace();
				return XmlUtil.tajikErrorLoggingXml ("Date parse error");
			}
			...
			Encounter e = new Encounter (encId, EncounterType.PATIENT_MONITORING, locationId, encounterdateentered, encounterStartDate, encounterEndDate, "");
			...
			EncounterResults dateResult = ModelUtil.createEncounterResult (e, "start_treatment".toUpperCase (), startDateTreatment);
			encounters.add (dateResult);
			
			EncounterResults encounterbaselineSmear = ModelUtil.createEncounterResult (e, "baselinesmear".toUpperCase (), baselineSmearResult.toUpperCase());
			encounters.add (encounterbaselineSmear);
			/*
			EncounterResults encounterSmearMonth2 = ModelUtil.createEncounterResult (e, "SmearResultMonth2".toUpperCase (), monthSmear2.toUpperCase());
			encounters.add (encounterSmearMonth2);
			
			EncounterResults encounterSmearMonth3 = ModelUtil.createEncounterResult (e, "SmearResultMonth3".toUpperCase (), monthSmear3.toUpperCase());
			encounters.add (encounterSmearMonth3);
			
			EncounterResults encounterSmearMonth4 = ModelUtil.createEncounterResult (e, "SmearResultMonth4".toUpperCase (), monthSmear4.toUpperCase());
			encounters.add (encounterSmearMonth4);
			
			EncounterResults encounterSmearMonth5 = ModelUtil.createEncounterResult (e, "SmearResultMonth5".toUpperCase (), monthSmear5.toUpperCase());
			encounters.add (encounterSmearMonth5);
			
			EncounterResults encounterSmearMonth6 = ModelUtil.createEncounterResult (e, "SmearResultMonth6".toUpperCase (), monthSmear6.toUpperCase());
			encounters.add (encounterSmearMonth6);
			
			EncounterResults encounterSmearMonth6Cat2 = ModelUtil.createEncounterResult (e, "SmearResultMonth6Cat2".toUpperCase (), monthSmear6Cat2.toUpperCase());
			encounters.add (encounterSmearMonth6Cat2);
			
			EncounterResults encounterSmearMonth7 = ModelUtil.createEncounterResult (e, "SmearResultMonth7".toUpperCase (), monthSmear7.toUpperCase());
			encounters.add (encounterSmearMonth7);
			
			EncounterResults encounterSmearMonth9 = ModelUtil.createEncounterResult (e, "SmearResultMonth9".toUpperCase (), monthSmear9.toUpperCase());
			encounters.add (encounterSmearMonth9);
			*/
			
			EncounterResults encounterSmearMonth9 = ModelUtil.createEncounterResult (e, "SmearResult".toUpperCase (), mr.getSmearResult ());
			encounters.add (encounterSmearMonth9);
			
			EncounterResults encounterCurrentMonth = ModelUtil.createEncounterResult (e, "currentMonth".toUpperCase (), monthTreatment.toUpperCase());
			encounters.add (encounterCurrentMonth);

			EncounterResults encounterPatientFeel = ModelUtil.createEncounterResult (e, "patientFeel".toUpperCase (), mr.getPatientFeel ());
			encounters.add (encounterPatientFeel);
			
			EncounterResults encounterPatientSideEffect = ModelUtil.createEncounterResult (e, "experiencedSideEffect".toUpperCase (), mr.getExperienceSideEffects());
			encounters.add (encounterPatientSideEffect);
			
			if(patientSideEffects!= null){
			
			EncounterResults encounterPatientSideEffects = ModelUtil.createEncounterResult (e, "patientSideEffects".toUpperCase (), mr.getPatientSideEffects ());
			encounters.add (encounterPatientSideEffects);}
			
			if(patientConsult!=null){
			
			EncounterResults encounterPatientConsult = ModelUtil.createEncounterResult (e, "patientConsult".toUpperCase (), mr.getPatientConsult ());
			encounters.add (encounterPatientConsult);}
			
			if(healthFacility!=null){
			
			EncounterResults encounterHealthFacility = ModelUtil.createEncounterResult (e, "healthFacility".toUpperCase (), mr.getHealthFacility ());
			encounters.add (encounterHealthFacility);}
			
			EncounterResults encounterMissMedication = ModelUtil.createEncounterResult (e, "missMedication".toUpperCase (), mr.getMissMedication ());
			encounters.add (encounterMissMedication);
			
			if(durationMissMedication!=null){
			
			EncounterResults encounterDurationMissMedication = ModelUtil.createEncounterResult (e, "durationMissMedication".toUpperCase (), mr.getDurationMissMedication ());
			encounters.add (encounterDurationMissMedication);}
			
			EncounterResults encounterMedicineWrappers = ModelUtil.createEncounterResult (e, "medicineWrappers".toUpperCase (), mr.getMedicineWrappers ());
			encounters.add (encounterMedicineWrappers);
			
			if(doseConsumption!=null){
			
			EncounterResults encounterdoseConsumption = ModelUtil.createEncounterResult (e, "doseConsumption".toUpperCase (), doseConsumption.toUpperCase());
			encounters.add (encounterdoseConsumption);}
	...
	private String handleFormQuery ()
	{
		...
		else if (requestType.equals (RequestType.MONITORING_FORM))
		{
			xml = getMonitoringFormInfo (id);
		}
		else if (requestType.equals (RequestType.BASELINE_FORM))
		{
			xml = getBaselineInfo (id);
		}
	...
	private String getBaselineInfo (String id)
	{
		String xml = null;
		...
		try
		{
			screening = ssl.findScreeningByPatientID(id);
		}
		catch (Exception e)
		{
			e.printStackTrace ();
			return XmlUtil.createErrorXml ("PATIENT_DETAILS_NOT_FOUND"+"/"+id);
		}
		
		
		try
		{
			pdetails = ssl.findPatientByPatientID(id);
		}
		catch (Exception e2)
		{
			e2.printStackTrace ();
		...
		try{
			...
			long dno =ssl.count ("baselinedetails", "where patientId='" + id + "'");
			if (dno != 0)
				return XmlUtil.createErrorXml ("BASELINE_DETAILS_ALEADY_FILLED");	
		}
		catch (Exception e2)
		{
			e2.printStackTrace ();
		...
		//get date of treatment initiation
		
		Date dateTreatmentInitiation = pdetails.getStartTreatment ();
		
		//converting date of treatment start in string
		
		String DATE_FORMAT_NOW = "yyyy-MM-dd";
	    SimpleDateFormat sdf = new SimpleDateFormat(DATE_FORMAT_NOW);
	    String stringDate = sdf.format(dateTreatmentInitiation);
		...
		Element treatmentNode = doc.createElement ("treatmentinitate");
		Text treatmentValue = doc.createTextNode (stringDate);
		treatmentNode.appendChild (treatmentValue);

		responseNode.appendChild (treatmentNode);
	...
	private String doBaseline()
	{
		String patientID=request.getParameter("parId");
		String startDate =request.getParameter("startdate");
		String startDateTreatment = request.getParameter ("sdate");
		String startTime = request.getParameter ("stime");
		String endTime = request.getParameter ("etime");
		String weight = request.getParameter ("weight");
		String locationId = null;
		String baseSputum = request.getParameter ("basesputum");
		String baseChest = request.getParameter ("basechest");
		String otherXraySite = request.getParameter ("otherxraysite");
		String resultXray = request.getParameter ("resultxray");
		String baseGenexpert = request.getParameter ("basegenexpert");
		String drugsensitive = request.getParameter ("drugsensitive");
		String typePatient = request.getParameter ("typepatient");
		String patientCategory = request.getParameter ("patientcategory");
		String regimen = request.getParameter ("regimen");
		String fixedDose = request.getParameter ("fixedDose");
		String strepto = request.getParameter ("strepto");
		String healthWorker = request.getParameter ("healthWorker");
		String pid= null;
        ...
        try{
			...
			long dno =ssl.count ("baselinedetails", "where patientId='" + patientID + "'");
			if (dno != 0)
				return XmlUtil.createErrorXml ("BASELINE_DETAILS_ALEADY_FILLED");	
		}
		catch (Exception e2)
		...
		BaselineDetails bsd = new BaselineDetails();
		
		bsd.setPatientId(patientID);
		try {
			if (startDateTreatment != null)
				bsd.setTreatmentInitiation(DateTimeUtil.getDateFromString (startDateTreatment, DateTimeUtil.FE_FORMAT_TRUNC));
			else
				bsd.setTreatmentInitiation(patientDetails.getStartTreatment ());
			bsd.setStartTimeForm(DateTimeUtil.getDateFromString (startDate+" "+startTime, DateTimeUtil.FE_FORMAT));
			bsd.setEndTimeForm(DateTimeUtil.getDateFromString (startDate+" "+endTime, DateTimeUtil.FE_FORMAT));
			
		} catch (ParseException e) {
			
			e.printStackTrace();
			return XmlUtil.createErrorXml ("Date Parse Error Occurred");
		}
		
		bsd.setChwId(healthWorker);
		...
		else if(areRussianStringsEqual(baseSputum,"1-9 КУБ"))
			bsd.setBaselineSputum("1-9 AFB");
		else
		...
		bsd.setRegimen(regimen);
		bsd.setFixedDose(fixedDose);
		bsd.setStrepto(strepto);
		...
		Encounter e = new Encounter (encId, EncounterType.BASELINE, locationId, encounterdateentered, encounterStartDate, encounterEndDate, "");
		...
		EncounterResults encounterDistrict = ModelUtil.createEncounterResult (e, "baselinesputum".toUpperCase (), bsd.getBaselineSputum ());
		encounters.add (encounterDistrict);
		
		EncounterResults encounterFlat = ModelUtil.createEncounterResult (e, "baselinechest".toUpperCase (), bsd.getBaselineChest ());
		encounters.add (encounterFlat);
		
		EncounterResults encounterXraySite = ModelUtil.createEncounterResult (e, "otherxraysite".toUpperCase (), bsd.getOtherXraySite ());
		encounters.add (encounterXraySite);
		
		EncounterResults encounterXrayResult = ModelUtil.createEncounterResult (e, "resultxray".toUpperCase (), bsd.getXrayResult ());
		encounters.add (encounterXrayResult);
		
		EncounterResults encounterHouse = ModelUtil.createEncounterResult (e, "baselinegenexpert".toUpperCase (), bsd.getBaselineGeneXpert ());
		encounters.add (encounterHouse);

		EncounterResults encounterStreet = ModelUtil.createEncounterResult (e, "Genexpertdrugsensitive".toUpperCase (), bsd.getGeneXpertDrugSensitivity ());
		encounters.add (encounterStreet);

		EncounterResults encounterPhone1 = ModelUtil.createEncounterResult (e, "typeofpatient".toUpperCase (), bsd.getTypePatient ());
		encounters.add (encounterPhone1);
		
		EncounterResults encounterPhoneOwner1 = ModelUtil.createEncounterResult (e, "patientcategory".toUpperCase (), bsd.getPatientCategory ());
		encounters.add (encounterPhoneOwner1);
		
		EncounterResults encounterPhone2 = ModelUtil.createEncounterResult (e, "regimen".toUpperCase (), regimen.toUpperCase());
		encounters.add (encounterPhone2);
		
		EncounterResults encounterPhoneOwner2 = ModelUtil.createEncounterResult (e, "fixeddose".toUpperCase (), fixedDose.toUpperCase());
		encounters.add (encounterPhoneOwner2);
		
		if (regimen.equals ("RHZES")){
			EncounterResults encounterMarital = ModelUtil.createEncounterResult (e, "strepto".toUpperCase (), strepto.toUpperCase());
			encounters.add (encounterMarital);
		}
	...
	private String doPatientRegistration()
	{
		...
		String dobField = request.getParameter ("dob");
		String gender = request.getParameter ("gender"); 
		String nominateVolunteer = request.getParameter ("nominatevolunteer");
		String relationship = request.getParameter ("relationship");
		String relationother = request.getParameter ("relationother");
		String maritalStatus = request.getParameter ("maritalstatus");
		String education = request.getParameter ("education");
		String incomeFamilyMember = request.getParameter ("incomeFamilyMember");
		String healthWorker = request.getParameter ("healthWorker");
		String pid = null;
		String locationId  = null;
		try
	    ...
	    try
		{
			long pno = ssl.count ("patientdetails", "where pid='" + patientID + "'" );
			if (pno != 0)
				return XmlUtil.createErrorXml ("PATIENT_DETAILS_ALREADY_FILLED"+"/"+patientID);
		}
		catch (Exception e3)
		{
		...
		pdetails.setPid(patientID);
		try {
			pdetails.setStartTreatment(DateTimeUtil.getDateFromString (startDateTreatment, DateTimeUtil.FE_FORMAT_TRUNC));
			pdetails.setStartTimeForm(DateTimeUtil.getDateFromString (startDate+" "+startTime, DateTimeUtil.FE_FORMAT));
			pdetails.setEndTimeForm(DateTimeUtil.getDateFromString (startDate+" "+endTime, DateTimeUtil.FE_FORMAT));
			pdetails.setdob(DateTimeUtil.getDateFromString (dobField, DateTimeUtil.FE_FORMAT_TRUNC));
		...
		pdetails.setAddressDistrict(district.toUpperCase());
		pdetails.setAddressFlat(flat);
		pdetails.setAddressHouse(house);
		pdetails.setAddressStreet(street.toUpperCase());
		pdetails.setFirstName (firstName.toUpperCase ());
		pdetails.setLastName (lastName.toUpperCase ());
		
		if(areRussianStringsEqual(gender,"Муж"))
			pdetails.setGender ("M");
		else
			pdetails.setGender ("F");
		
		
		pdetails.setHomePhone (homePhone);
		pdetails.setPhone1(phone1);
		
		if(areRussianStringsEqual(nominateVolunteer,"Да"))
			pdetails.setNominateVolunteer ("YES");
		else
			pdetails.setNominateVolunteer ("NO");
			
		if (pdetails.getNominateVolunteer().equals ("YES")){ 
			if(areRussianStringsEqual(relationship,"Сын/дочь"))
		...
		else if(areRussianStringsEqual(maritalStatus,"Разведен/а"))
			pdetails.setMaritalStatus("DIVORCED");
		else if(areRussianStringsEqual(maritalStatus,"Живу раздельно"))
			pdetails.setMaritalStatus("WIDOW");
		else if(areRussianStringsEqual(maritalStatus,"Вдовец/Вдова"))
			pdetails.setMaritalStatus("WIDOWER");
		else
		...
		pdetails.setHealthWorkerID(healthWorker);
		
		pdetails.setActive("1");
		pdetails.setInactiveDate(null);
		...
		Encounter e = new Encounter (encId, EncounterType.PATIENT_DETAILS, locationId, encounterdateentered, encounterStartDate, encounterEndDate, "");
		...
		EncounterResults startTreatment = ModelUtil.createEncounterResult (e, "Start_treatment".toUpperCase (), startDateTreatment);
		encounters.add (startTreatment);
		
		EncounterResults encounterDistrict = ModelUtil.createEncounterResult (e, "Address_district".toUpperCase (), district.toUpperCase());
		encounters.add (encounterDistrict);
		
		EncounterResults encounterFlat = ModelUtil.createEncounterResult (e, "address_flat".toUpperCase (), flat.toUpperCase());
		encounters.add (encounterFlat);
		
		EncounterResults encounterHouse = ModelUtil.createEncounterResult (e, "address_house".toUpperCase (), house.toUpperCase());
		encounters.add (encounterHouse);

		EncounterResults encounterStreet = ModelUtil.createEncounterResult (e, "address_Street".toUpperCase (), street.toUpperCase());
		encounters.add (encounterStreet);

		EncounterResults encounterHomePhone = ModelUtil.createEncounterResult (e, "home_phone".toUpperCase (), homePhone.toUpperCase());
		encounters.add (encounterHomePhone);
		
		EncounterResults encounterPhone1 = ModelUtil.createEncounterResult (e, "address_phone".toUpperCase (), phone1.toUpperCase());
		encounters.add (encounterPhone1);
		
		EncounterResults encounterFirstName = ModelUtil.createEncounterResult (e, "first_name".toUpperCase (), firstName.toUpperCase());
		encounters.add (encounterFirstName);
		
		EncounterResults encounterLastName = ModelUtil.createEncounterResult (e, "last_name".toUpperCase (), lastName.toUpperCase());
		encounters.add (encounterLastName);
		
		EncounterResults encounterDOB = ModelUtil.createEncounterResult (e, "Date_of_Birth".toUpperCase (), dobField);
		encounters.add (encounterDOB);
		
		EncounterResults encounterGender = ModelUtil.createEncounterResult (e, "gender".toUpperCase (), pdetails.getGender ());
		encounters.add (encounterGender);
		
		EncounterResults encounterNominateVolunteer = ModelUtil.createEncounterResult (e, "nominate_volunteer".toUpperCase (), pdetails.getNominateVolunteer ());
		encounters.add (encounterNominateVolunteer);
		
		EncounterResults encounterMarital = ModelUtil.createEncounterResult (e, "marital".toUpperCase (), pdetails.getMaritalStatus ());
		encounters.add (encounterMarital);
		
		EncounterResults encounterEducation = ModelUtil.createEncounterResult (e, "education".toUpperCase (), pdetails.getEducation ());
		encounters.add (encounterEducation);
		
		EncounterResults encounterIncome = ModelUtil.createEncounterResult (e, "income".toUpperCase (), pdetails.getIncomeFamilyMember ());
		encounters.add (encounterIncome);
				
		if(pdetails.getNominateVolunteer ().equals ("YES")) {
			
			EncounterResults encounterRelationship = ModelUtil.createEncounterResult (e, "relationship_family".toUpperCase (), pdetails.getRelationshipFamily ());
			encounters.add (encounterRelationship);
		...
		//TODO: set reminder for patient! ?? reminder id! ??UserID
		
		PatientReminder pr = new PatientReminder();
		pr.setPatientDetails (pdetails);
		Reminder r1 = ssl.findReminder("1");
		pr.setReminder (r1);
		...
		pr.setGapInHours (2);
		pr.setLastUpdated (d);
		pr.setCreatedByUserName (ssl.getCurrentUser ());
		pr.setCreatedDate (d);
		pr.setLastEditedByUserId (ssl.getCurrentUser ());
...
*/
	...
	public String getMonitoringFormInfo(String id)
	{
		String xml = null;
		...
		{
			e.printStackTrace ();
			return XmlUtil.createErrorXml ("SCREENING_FORM_NOT_FOUND"+"/"+id);
		}
		...
		try
		{
			bdetails = ssl.findBaselineByPatientID(id);
		}
		catch (Exception e2)
		{
			e2.printStackTrace ();
			return XmlUtil.createErrorXml ("BASELINE_DETAILS_NOT_FOUND"+"/"+id);
		}
		//get date of treatment initiation
		//get baseline smear result
		
		String baselineSmear = bdetails.getBaselineSputum();
		Date dateTreatmentInitiation = bdetails.getTreatmentInitiation();
		...
		long mno = 0;
		String previousMR = "no";
		try{
			
			mno = ssl.count ("monitoringresults", "where patientId='" + id + "'"); 
		}catch(Exception e ){
		...
		if(mno != 0)
			previousMR = ssl.getPreviousMonitiringResultInString (id);
		...
		Element baseNode = doc.createElement ("baselinesmear");
		Text baseValue = doc.createTextNode (baselineSmear);
		baseNode.appendChild (baseValue);

		responseNode.appendChild (baseNode);
		...
		Element categoryNode = doc.createElement ("previousMR");
		Text currentValue = doc.createTextNode (previousMR);
		categoryNode.appendChild (currentValue);

		responseNode.appendChild (categoryNode);
		doc.appendChild (responseNode);
...
//			GeneXpertResultsAuto gxp = null;
			...
			if(gxpCart==null && gxpPatient==null) 
			{
				...
				try {
					...
					ssl.saveGeneXpertResults(gxpU, gxpU.getIsPositive());//,operatorId,pcId,instrumentSerial,moduleId,cartridgeId,reagentLotId);
					return XmlUtil.createSuccessXml();
				} catch (Exception e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
			...
			try {
				//ssl.updateGeneXpertResultsAuto(gxp, gxp.getIsPositive(),operatorId,pcId,instrumentSerial,moduleId,cartridgeId,reagentLotId);
				ssl.updateGeneXpertResults(gxpNew, gxpNew.getIsPositive());
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
