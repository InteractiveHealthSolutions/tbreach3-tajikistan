...
public class ServerServiceImpl
{
	...
	public Boolean updateGeneXpertResults(GeneXpertResults geneXpertResults, boolean isTBPositive) throws Exception
	{
		...
		SMSUtil.util.sendAlertsOnAutoGXPResults(geneXpertResults, isTBPositive);
		return result;
	}
	
	public Boolean saveGeneXpertResults(GeneXpertResults geneXpertResults, boolean isTBPositive) throws Exception
	{
...
/*	public Boolean verifySecretAnswer (String userName, String secretAnswer)
	...
	public Long dailyReminderCount(String dateFilter) throws Exception
	{
		Object obj = HibernateUtil.util.selectObject("select count(record_num) from reminder_history where reminder_status='SENT' AND reminder_id = '1' AND "+dateFilter);
		return Long.parseLong(obj.toString());		
	}
	
	public Long dailyReminderCountPerDate(String dateFilter, int noOfReminder) throws Exception
	{
		Object obj = HibernateUtil.util.selectObject("select count(record_num) from reminder_history where reminder_status='SENT' AND reminder_id = '1' AND reminder_num_for_the_day = '"+noOfReminder+"' AND "+dateFilter);
		return Long.parseLong(obj.toString());		
	}
	
	public PatientResponse[] dailyResponseForDate(String dateFilter) throws Exception
	{
		Object[] objs = HibernateUtil.util.findObjects ("from PatientResponse where " + dateFilter);
		if(objs == null) return null;
		PatientResponse[] patientResponses = new PatientResponse[objs.length];
		for (int i = 0; i < objs.length; i++)
			patientResponses[i] = (PatientResponse) objs[i];
		return patientResponses;	
	}
	
	public int[] countResponsePerIlteration (String dateFilter) throws Exception{
		
		PatientResponse[] pr = dailyResponseForDate (dateFilter);
		int count[] = new int[3];
		count[0]=0;
		count[1]=0;
		count[2]=0;
		...
		for(PatientResponse patRes: pr){
			
			Date d = patRes.getRecieveDate ();
			int hour = d.getHours ();
	...
	public Long dailyPatientResponseCount(String dateFilter) throws Exception
	{
		Object obj = HibernateUtil.util.selectObject("select count(record_num) from patient_response where "+ dateFilter);
		return Long.parseLong(obj.toString());		
	}
	...
	public Long workingDaysReminder(){
		Object obj = HibernateUtil.util.selectObject("select DATEDIFF(now(),'2014-04-19 23:59:59')");
		return Long.parseLong(obj.toString());		
	}
...
/*	public String getSecretQuestion (String userName)
	...
	public PatientReminder findPatientReminder (String patientID)
	{
		return (PatientReminder) HibernateUtil.util.findObject ("from PatientReminder where p_id='" + patientID + "'");
	}
	...
	{
		return (Reminder) HibernateUtil.util.findObject ("from Reminder where reminder_id='"+ID+"'");
	}
	...
	public PatientDetails findPatientByPatientID (String patientID)
	{
		return (PatientDetails) HibernateUtil.util.findObject ("from PatientDetails where pid='" + patientID + "'");
	}
	
	public BaselineDetails findBaselineByPatientID (String patientID)
	{
		return (BaselineDetails) HibernateUtil.util.findObject ("from BaselineDetails where patientId='" + patientID + "'");
	}
	
	
	public MonitoringResults[] findMonitoringResultsByPatientId (String patientID)
	{
		Object[] objects = HibernateUtil.util.findObjects ("from MonitoringResults where patientId='" + patientID + "'");
		MonitoringResults[] mResults = new MonitoringResults[objects.length];
		for (int i = 0; i < mResults.length; i++)
			mResults[i] = (MonitoringResults) objects[i];
		return mResults;
	}
	
	public String getPreviousMonitiringResultInString(String patientID){
		
		String result = "";
		MonitoringResults[] mrs = findMonitoringResultsByPatientId (patientID);
		for (MonitoringResults mr : mrs)
		{
			result = result + mr.getCurrentMonth () + " " + mr.getSmearResult () + "/";
		}
	...
	{
		Object obj = HibernateUtil.util.findObject ("from Users where UserName='" + userName + "'");
	...
	public MonitoringResults findMonitoringResultsByPatientIDAndMonth (String patientID, String monthTreatment)
	{
		return (MonitoringResults) HibernateUtil.util.findObject ("from MonitoringResults where PatientID='" + patientID + "' AND CurrentMonth='"+monthTreatment+"'");
	}
